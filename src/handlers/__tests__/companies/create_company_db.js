...
describe('Create company.', () => {
  ...
  beforeAll(async () => {
    ...
    const { createCompany } = await import('../../company/index.js')
  ...
  it('Should create a company.', async () => {
    ...
    const daysToRetainData = 0
    const capabilities = ['detect_face']
    ...
    // Act
    await createCompanyHandler(ctx, request, response)
    ...
    expect(attributes.displayName).toBe(displayName)
    expect(attributes.daysToRetainData).toBe(daysToRetainData)
    expect(attributes.capabilities).toStrictEqual(capabilities)
  })
})
