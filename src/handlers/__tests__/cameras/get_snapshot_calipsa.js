...
const { default: getSnapshotCalipsa } = await import('../../calipsa/get_snapshot.js')
...
const { getSnapshotUrl, getTamperingRefUrl } = await import('../../../common/s3.js')
...
describe('Request Errors', () => {
  it('Should return not found if the camera is not found', async () => {
    ...
    // Act
    await getSnapshotCalipsa(ctx, request, response)
...
describe('Successful Redirect', () => {
  ...
  it('Should redirect if a URL is successfully created for a ref image', async () => {
    // Arrange
    const goodUrl = 'http://foo.bar/stuff'
    const cacheControl = 'max-age=3600'
    retrieveCameraData.mockResolvedValue(mockCamera)
